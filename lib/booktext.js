...
Book.prototype.getPageByAbbr = function (abbr) {
  for (let i = 0; i < this.book.length; i++) {
    let curr = this.book[i].abbr;
    if (Array.isArray(curr) === true) {
      for (let j = 0; j < curr.length; j++) {
        if (curr[j].toLowerCase() === abbr.toLowerCase()) return this.book[i];
      }
    } else {
      if (curr.toLowerCase() === abbr.toLowerCase()) return this.book[i];
    }
  }
  return null;
};
...
Book.prototype.getPageByName = function (name) {
  let ab = this.getPageByAbbr(name);
  if (ab) return ab;
  for (let i = 0; i < this.book.length; i++) {
...
Book.prototype.search = function (terms) {
  let ret = [];
  for (let i = 0; i < this.book.length; i++) {
    let curr = this.book[i];
    let searchable = [curr.title, curr.subtitle, curr.text].join(" ");
    let containsAllTerms = true;
    for (let j = 0; j < terms.length; j++) {
      if (searchable.includes(terms[j]) === false) {
        containsAllTerms = false;
      }
    }
    if (containsAllTerms === true) {
      ret.push(i);
    }
  }
  return ret;
...
Book.prototype.pageToPretty = function (page) {
  ...
  ret += page.text + "\n";
  if (page.abbr)
    ret += "Abbreviation Code: `" + (Array.isArray(page.abbr) === true ? page.abbr[0] : page.abbr) + "`";
  return ret;
};
...
Book.prototype.tableofcontents = function () {
  ...
  for (let i = 0; i < this.book.length; i++) {
    ret += "" + pad(i + 1, this.book.length) + " " + this.book[i].title + "\n";
  }
  ret += "```";
  return ret;
};

Book.prototype.showSearch = function (terms) {
  let results = this.search(terms);
  if (results.length === 0) {
    return "No results found."
  } else {
    let ret = "**Results:**\n```";
    for (let i = 0; i < results.length; i++) {
      ret += "" + pad(results[i] + 1, this.book.length) + " " + this.book[results[i]].title + "\n";
    }
    ret += "```";
    return ret;
  }
...
function enchantmentCreator(book) {
  return (args, send, server) => {
    if (server.dnd) {
      let pageNumber = Number(args[0]);
      if (args[0].toLowerCase() === "toc") {
        send(book.tableofcontents());
      } else if (args[0].toLowerCase() === "search") {
        send(book.showSearch(args.slice(1)));
      } else if (isNaN(pageNumber) === false) {
        let page = book.getPageByNum(pageNumber);
        if (page)
          send(book.pageToPretty(page));
      } else {
        let page = book.getPageByName(args.join(" "));
        if (page)
          send(book.pageToPretty(page));
      }
    }
