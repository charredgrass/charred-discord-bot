...
class Game {
  ...
  /**
   * @param {Snowflake} fromID
   * @param {Snowflake} toID
   * @param {Number} amount
   * @param {Function} cb
   */
  give(fromID, toID, amount, cb) {
    let from = this.getPlayer(fromID);
    let to = this.getPlayer(toID);
    if (!to) {
      cb("Player not found.")
    } if (from.credits >= amount) {
      from.removeCredits(amount);
      to.addCredits(amount);
      cb("Sent " + amount + " credits to " + to.name);
    } else {
      cb("Not enough credits.");
    }
  }
}
