...
function generateStrip() {
  ...
  for (let i = 0; i < slotEmoji.length; i++) {
    ...
    ret.push(":poop:"); //one dud for each tier
  }
...
function getSpinMultiplier(spinResult) {
  ...
  switch (matching[3]) {
    case 3:
      ...
      break;
    case 2:
      isBetter(ret, 10, "2 of the same " + slotEmoji[3], "2t3m");
      break;
  }
  switch (matching[2]) {
    case 3:
      isBetter(ret, 500, "3 of the same " + slotEmoji[2], "3t2m");
      break;
    case 2:
      isBetter(ret, 8, "2 of the same " + slotEmoji[2], "2t2m");
      break;
  }
  switch (matching[1]) {
    case 3:
      isBetter(ret, 40, "3 of the same " + slotEmoji[1], "3t1m");
      break;
    case 2:
      isBetter(ret, 3, "2 of the same " + slotEmoji[1], "2t1m");
      break;
  }
  switch (matching[0]) {
    case 3:
      isBetter(ret, 15, "3 of the same " + slotEmoji[0], "3t0m");
      break;
    case 2:
      isBetter(ret, 1, "2 of the same " + slotEmoji[0], "2t0m");
      break;
  }
  switch (nonMatching[3]) {
    case 3:
      isBetter(ret, 50, "3 of any " + slotEmoji[3], "3t3");
      break;
    case 2:
      isBetter(ret, 5, "2 of any " + slotEmoji[3], "2t3");
      break;
  }
  switch (nonMatching[2]) {
    case 3:
      isBetter(ret, 45, "3 of any " + slotEmoji[2], "3t2");
      break;
    case 2:
      isBetter(ret, 4, "2 of any " + slotEmoji[2], "2t2");
      break;
  }
  ...
  //   case 3:
  //     isBetter(ret, 4, "3 of any " + slotEmoji[0], "3t0");
  //     break;
  // }
  return ret;
}
...
function testAFuckTon() {
  ...
  for (let x in data) {
    if (data.hasOwnProperty(x)) {
      data[x] = ( (data[x] / NUM_TESTS)) * 100;
    }
  }
  // console.log(JSON.stringify(data, null, 4));
  let display = [];
  for (let key in data) {
    display.push({"type": key, "amount": data[key]});
  }
  display.sort((a, b) => {
    return b.amount - a.amount;
  });
  for (let a of display) {
    console.log(a.type + " " + a.amount);
  }
  console.log(bal / NUM_TESTS);
}
