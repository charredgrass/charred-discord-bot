...
class BlackjackGame extends SinglePlayerGame {
   ...
   */
  status(callback) {
    let ret = "Blackjack! " + this.player + "'s game, " + this.bet + " bet \n";
    ret += "Dealer's Hand " + this.dealerHand.toString() + " ??";
    ret += "Player's Hand " + handToString(this.playerHand);
    callback(ret);
  }
  ...
  finish(callback) {
    ...
    ret += "Player's Hand " + handToString(this.playerHand);
    let winner = chooseWinner(this.dealerHand, this.playerHand);
    switch (winner) {
