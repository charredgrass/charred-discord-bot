...
/**Call the WoW API and get the price for a WoW Token.
 * @param {string} token The access token.
 * @param {function} cb Callback for success.
 * @param {function} ecb Callback for failure.
 */
...
function getProgressionData(chardata, names) {
...
/**Pads a single digit number to have a length of 2.
 * @param {number} num
 * @returns {string}
 */
...
function prettyProg(prog, names, abbrevs, realm, character) {
  ...
  ret += "\n```";
  if (prog.kills[3][3] > 0) { //update first index for mythic raids
...
/**Given client credentials, request an authorization token from the
 *  Blizzard API.
 * @param {string} clientID 
 * @param {string} clientSecret
 * @param {function} cb Callback, called on success.
 */
