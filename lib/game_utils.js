...
function betterNonBust(hand) {
  let x = totalValue(this.scoredata.dealer_hand, true);
  let y = totalValue(this.scoredata.dealer_hand, false);
  if (x > 21) return y;
  else return x;
}
...
GameData.prototype.startGame = function(id, type, bet) {
  for (let i = 0; i < this.data.players.length; i++) {
    if (this.data.players[i].id == id && !(this.data.players[i].currgame)) {
      this.data.players[i].currgame = new CurrentGame(type, bet, this.data.players[i].id, this).start();
    }
  }
}
