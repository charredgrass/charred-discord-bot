  ...
  for (let i = 0; i < this.data.players.length; i++) {
    if (this.data.players[i].id == id) {
      if (this.data.players[i].last + (1000 * 60 * 60 * 4) > Date.now()) {
        cb("Cannot refill now, try again later. You need to wait 240 minutes. between refills. You last got free coins " + millisToMinutes(Date.now() - this.data.players[i].last) + " minutes ago");
      } else {
